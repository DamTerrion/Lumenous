...
def imp_conf (conf_name='ndxf.conf'):
    try:
        configuration_file = open(conf_name)
        for line in configuration_file:
            if line.find('=') > 0:
                ...
                if (set_name.isalnum() and
                    ...
                    config[sett_name.lower()] = set_value
    except Exception:
