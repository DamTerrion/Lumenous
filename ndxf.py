...
def ndxf (dxf_name):
    ...
    while not (Param == '  2\n' and Value == 'ENTITIES\n'):
        ...
        while not (Param == '  0\n' and Value == 'ENDSEC\n'):
            ...
            new += '  0\n'+'ENDSEC\n'+'  0\n'+'EOF\n'
    dxf.close()
    rename (dxf_name, dxf_name[:-4]+'.bak')
    # После закрытия считываемого файла, его расширение меняется на .bak
    # Так как расширение исходного файла сменилось, создаётся новый файл с исходным именем
    dxf = open (dxf_name, 'w')    
    dxf.write(new)
    dxf.close()
    print ('Well done!')
